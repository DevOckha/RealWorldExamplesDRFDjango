...
class ArticleViewSet(mixins.CreateModelMixin, mixins.ListModelMixin, mixins.RetrieveModelMixin, viewsets.GenericViewSet):
    ...
    def get_queryset(self):
        ...
        author = self.queryset.query_params.get('author', None)
        if author is not None:
            queryset = queryset.filter(author__user__username=author)
        
        tag = self.queryset.query_params.get('tag', None)
        if tag is not None:
            queryset = self.queryset.filter(tags__tag = tag)
        
        favorited_by = self.queryset.query_params.get('favorited', None)
        if favorited_by is not None:
            queryset = self.queryset.filter(favorited_by__user__username=favorited_by)
...
class CommentListCreateAPIView(generics.ListCreateAPIView):
    ...
    permission_classes = (IsAuthenticatedOrReadOnly, )
    queryset = Comment.objects.select_related('article', 'article__author', 'article__author__user', 'author', 'author__user')
    ...
    def create(self, request, article_slug):
...
class CommentDestroyAPIView(generics.DestroyAPIView):
    ...
    def destroy(self, request, article_slug, comment_pk=None):
...
class ArticleFavoriteAPIView(APIView):
    ...
    def delete(self, request, article_slug=None):
        ...
        try:
            ...
            raise NotFound("An article with this slug was not exist")
...
class TagListAPIView(generics.ListAPIView):
    ...
    def list(self, request, *args, **kwargs):
        ...
        return Response({'tags': serializer_data}, status=status.HTTP_200_OK)
